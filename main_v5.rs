...
// r2 coeficiente de concordancia
fn r2_concord(x: &[f64], y: &[f64], yh:&[f64]) -> (f64,f64) {
    let n = x.len();
    //let x_mean = x.iter().sum::<f64>() / n as f64;
    let y_mean = y.iter().sum::<f64>() / n as f64;
    //let slope = x.iter().zip(y.iter())
    //    .map(|(xi, yi)| (xi - x_mean) * (yi - y_mean))
    //    .sum::<f64>() / x.iter().map(|xi| (xi - x_mean).powi(2)).sum::<f64>();
    //let intercept = y_mean - slope * x_mean;
    //let y_pred = x.iter().map(|xi| slope * xi + intercept).collect::<Vec<_>>();
    let ssr = y.iter().zip(yh.iter())
        .map(|(yi, ypi)| (ypi - y_mean).powi(2))
        .sum::<f64>();
    let sst = y.iter().map(|yi| (yi - y_mean).powi(2)).sum::<f64>();
    let r22 = ssr / sst;
    let r22a = 1.0 - (((1.0-r22)*(n as f64-1.0))/(n as f64 -2.0-1.0));
    (r22,r22a)
}
...
fn main() {
    ...
    let mut wyh:Vec<f64> =Vec::new();
    for i in 0..x_test.len(){
        wx.push(  x_test[i]);
        wy.push(  y_test[i]);
        wyh.push( y_hat[i]);
    }
    let (r22,r22a)=r2_concord(&wx,&wy,&wyh);
    if r22.is_infinite() || r22.is_nan() || r22a.is_infinite() || r22a.is_nan() {}
    else{
    println!(" r2 = {}   r2_ajust = {}",r22,r22a);
    }
    ...
    for jf in csvf {
        ...
        let intercept = x[(variablesx, 0)];
        let coe = coeff[0];
        println!("\n Archivo {} |> Intensidad {:.3}% |>  fc = {} * edad + ({}) \n",jf,inten as f64 * fraccion, coe, intercept);
        inten =inten+1;
        //****
        // definir variables de trabajo
        let nr = bos.nrows();
