...
fn r2_concord(actual: &[f64], predicted: &[f64]) -> f64 {
    let n = actual.len();
    if n != predicted.len() || n <= 1 {
        println!("Error: no se puede calcular R2...");
    }

    let actual_mean = actual.iter().sum::<f64>() / n as f64;
    let sst = actual.iter().map(|&x| (x - actual_mean).powi(2)).sum::<f64>();

    let sse = actual.iter().zip(predicted.iter())
        .map(|(&a, &p)| (a - p).powi(2))
        .sum::<f64>(); // =rss
...
fn analisis() -> Vec<String>{
    Command::new("cmd")
    .args(&["/c", "cls"])
    .spawn()
    .expect("cls command no aplicable en esta situación ...")
    .wait()
    .expect("falla la espera ...");

    let mut salida=Vec::new();

    salida.push("\nBUAP: Enrique Buendia buendiaenr1@gmail.com\n".to_string());
    ...
    salida.push("Control de intensidad de la actividad vía FC\n\n".to_string());
    ...
    let nr = bos.nrows();
    let pnr = (bos.nrows() as f64 * 0.8).round() as usize; // 80% para entrenar
    let nc = bos.ncols();
    ...
    println!("x mínimo {} y máximo {} ", minimox, maximox);
    println!("y mínimo {} y máximo {} \n", minimoy, maximoy);
    
    // opción 1:  de 0 a FCmaxima *****************************************************************
    let rep: usize = (maximoy / 6 as f64) as usize;
    let fraccion = 100.0 / rep as f64; // porcentaje en o por bajo de el
    println!(
        " ::: OPCIÓN 1: Fractiles : {} , las fracciones son del {}",
        rep, fraccion
    );
    salida.push(" ::: Opción 1: 0 a FCmáx \n".to_string()); 
    println!(" ::: Rango 0 a FCmax\n"); 
    //for j in 1..rep {
    //    println!(
    ...
    let coe = coeff[0];
    println!("  GENERAL   fc = {} * edad + ({}) \n", coe, intercept);

    let y_hat = (x_test.clone() * &coeff).add_scalar(intercept);
    ...
    for i in 0..y_test.len() {
        residuales.push(((y_test[i] - y_hat[i]).abs()).abs());
        suma = suma + (y_test[i] - y_hat[i]).abs();
        suma2 = suma2 + (y_test[i] - y_hat[i]).powf(2.0);
    }
    // residuales
    println!(" Residuales (mejor si <= 3) : {:.3?}",residuales);
    residuales.clear();
    // cambiar de matriz a vector
    ...
    let r22=r2_concord(&wx,&wy);
    if r22.is_infinite() || r22.is_nan()  {}
    else{
        println!(" r2 = {} ",r22);
    }
    ...
    for jf in csvf {
        ...
        let intercept = x[(variablesx, 0)];
        let coe = coeff[0];
        println!("\n Archivo {} |> Intensidad {:.3}% |>  fc = {} * edad + ({}) \n",jf,iii[inten as usize] as f64 * fraccion, coe, intercept);
        let mut cadd1="Intensidad : ".to_string();
        let mut cadd2=(iii[inten as usize]as f64 *fraccion).to_string();
        let  cadd3=cadd1 + &cadd2;
        cadd1="  fc= ".to_string();
        cadd2=coe.to_string();
        let cadd3=cadd3 + &cadd1;
        let cadd3=cadd3 + &cadd2;
        cadd1=" * edad + (".to_string();
        let mut cadd3=cadd3 + &cadd1;

        cadd1=intercept.to_string();
        cadd2=" ) \n".to_string();
        cadd3=cadd3 + &cadd1;
        cadd3=cadd3 + &cadd2;
        salida.push(cadd3.to_string());
        ...
        println!("x mínimo {} y máximo {} ", minimox, maximox);
        println!("y mínimo {} y máximo {} \n", minimoy, maximoy);
        let caddd1="x mínimo ".to_string();
        let caddd2=minimox.to_string();
        let caddd3=caddd1 + &caddd2;
        let caddd1="  y máximo  ".to_string();
        let caddd2=maximox.to_string();
        let caddd1=caddd1 + &caddd2;
        let caddd3=caddd3 + &caddd1;
        let caddd3=caddd3 + &" \n";
        salida.push(caddd3.to_string());

        
        
        let caddd1="y mínimo ".to_string();
        let caddd2=minimoy.to_string();
        let caddd3=caddd1 + &caddd2;
        let caddd1="  y máximo  ".to_string();
        let caddd2=maximoy.to_string();
        let caddd1=caddd1 + &caddd2;
        let caddd3=caddd3 + &caddd1;
        let caddd3=caddd3 + &" \n";
        salida.push(caddd3.to_string());
        ...
        let y_hat = (x_test.clone() * &coeff).add_scalar(intercept);
        let y_hat1 = (x_train.clone() * &coeff).add_scalar(intercept);
        //println!("  y_estimada {}", y_hat.len());
        //println!("  y_prueba {}", y_test.len());
        //println!("mae: {}", mean_absolute_error(&y_test, &y_hat));
        let mut suma = 0.0;
        ...
        let mut residuales:Vec<f64>=Vec::new();
        for i in 0..y_test.len() {
            residuales.push((y_test[i] - y_hat[i]).abs());
            suma = suma + (y_test[i] - y_hat[i]).abs();
            suma2 = suma2 + (y_test[i] - y_hat[i]).powf(2.0); // RSS
    ...
    salida.push("\n\n ::: Opción 2: FCmáx - FCmin o basal \n".to_string()); 
    println!(" ::: Rango FCmax - FCmin,rep,basal\n");
    //for j in 1..rep {
    //    println!(
...
fn long_calculation() -> Vec<String> {
    //app::sleep(1.0);
    let s=analisis();
    s
}
...
fn main() {
    let a = App::default();
    let mut win = Window::default().with_size(900, 670).with_label("Intensidad");
    let mut disp = TextDisplay::new(5, 5, 890, 550, None);
    let mut btn = Button::default()
        .with_size(390, 30)
        .below_of(&disp, 10)
        .with_label("Mostrar");
    let mut btn1 = Button::default()
        .with_size(390, 30)
        //.center_of(&disp)
        .below_of(&disp, 60)
        .with_label("Salir");
    ...
    buf.append("Iniciando app\n");
    disp.set_buffer(buf.clone());

    btn.set_callback(move |b| {
        buf.append("trabajando...\n");
        b.deactivate();
        //std::thread::spawn({
            let mut buf = buf.clone();
            let mut b = b.clone();
            (move || {
                let result = long_calculation();
                for str11 in result {
                    buf.append(&str11.as_str());
                }
                buf.append("\n");
                b.activate();
            })()
