...
fn main() {
    ...
    for jf in csvf {
        ...
        let mut r2a=0.0;
        for i in 0..y_test.len(){
            prom1=prom1+y_test[i];
        }
        prom1=prom1/y_test.len() as f64;
        for i in 0..y_test.len() {
            suma = suma + (y_test[i] - y_hat[i]).abs();
            suma2 = suma2 + (y_test[i] - y_hat[i]).powf(2.0); // RSS
            tss=tss+(y_test[i]-prom1).powf(2.0);
        }
        r2=(tss-suma2)/tss;
        r2a=1.0-(((1.0-r2)*(y_test.len() as f64-1.0))/(y_test.len() as f64 -1.0-1.0));
        
        if r2.is_nan() || r2a.is_nan() || r2.abs().is_infinite() || r2a.abs().is_infinite() {
            println!(" No es posible calcular R2 ...");
        }else{
        println!("  R2= {}   R2_ajust= {}",r2,r2a);}
        let sumaf = suma.abs() / y_test.len() as f64;
        println!(" mae : {}", sumaf);
        let sumaf = suma2 / y_test.len() as f64;
        println!(" mse : {}", sumaf);
